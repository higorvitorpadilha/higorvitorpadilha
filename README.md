# 👋 Olá, eu sou Higor Padilha

**💻 Backend Developer | Cloud Engineer | DevOps Enthusiast**

<p align="left">
  <a href="https://www.linkedin.com/in/higor-padilha-41aaa4236/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>

  <a href="mailto:higorvitorpadilha@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>

  <a href="https://github.com/higorvitorpadilha" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

---

## 🚀 Sobre mim

Desenvolvedor focado em construir projetos reais e funcionais, atuando com backend, automação, cloud e DevOps.

Tenho experiência prática com:
- APIs REST
- Docker
- CI/CD
- AWS
- Azure
- JavaScript
- C#
- React
- Automação com Python
- Integração de sistemas

Também utilizo IA no desenvolvimento para aumentar produtividade, acelerar soluções e otimizar processos.

📍 Brasil  
🎯 Buscando oportunidade como Desenvolvedor Backend ou DevOps Júnior

---

## 💡 Stack Técnico

### 💻 Linguagens & Frameworks

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="45" height="45"/>
</p>

---

### ⚙️ Backend & DevOps

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original-wordmark.svg" width="45" height="45"/>
</p>

**Ferramentas:**  
GitHub Actions • CI/CD • AWS ECS • AWS ECR • REST APIs • Docker • Automação

---

## 📌 Projetos em Destaque

### 🔹 Baldan System
Sistema para centralizar dados e melhorar organização de informações utilizando React e AWS.

🔗 Projeto:  
https://github.com/higorvitorpadilha

---

### 🔹 Projeto CI/CD com AWS ECS
Pipeline automatizada utilizando GitHub Actions, Docker e AWS ECS seguindo práticas modernas de DevOps.

🔗 Repositório:  
https://github.com/higorvitorpadilha/Projeto-CI-CD

---

### 🔹 Job Application Bot
Automação para envio de candidaturas utilizando Python e integração de APIs.

🔗 Projeto:  
https://github.com/higorvitorpadilha

---

## 📊 GitHub Stats

<p align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=higorvitorpadilha&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true"/>

  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=higorvitorpadilha&layout=compact&langs_count=7&theme=tokyonight"/>
</p>

---

## 🎯 Objetivo

🚀 Buscando oportunidade como:
- Backend Júnior
- DevOps Júnior
- Estagiário em Desenvolvimento

---

## 🤖 IA no Desenvolvimento

Utilizo ferramentas como:
- ChatGPT
- Claude
- Gemini
- GitHub Copilot

para:
- acelerar desenvolvimento
- melhorar produtividade
- gerar soluções
- automatizar tarefas

---

## 📫 Contato

<p align="left">
  <a href="mailto:higorvitorpadilha@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>

  <a href="https://www.linkedin.com/in/higor-padilha-41aaa4236/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
</p>

---

<p align="center">
  <em>⭐ Obrigado por visitar meu perfil!</em>
</p>
